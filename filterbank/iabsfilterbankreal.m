function [f,relres,iter]=iabsfilterbankreal(s,g,a,varargin)
%IABSFILTERBANKREAL  Spectrogram inversion
%   Usage:  f=iabsfilterbankreal(s,g,a);
%           f=iabsfilterbankreal(s,g,a,Ls);
%           [f,relres,iter]=iabsfilterbankreal(...);
%
%   Input parameters:
...
%
%   IABSFILTERBANKREAL(s,g,a) attempts to invert a spectrogram computed by
%
%C     s = abs(dgt(f,g,a,M)).^2;
%
%   by an iterative method.
%
%   IABSFILTERBANKREAL(c,g,a,Ls) does as above but cuts or extends f to length Ls.
%
%   If the phase of the spectrogram is known, it is much better to use
%   IDGT.
%
%   [f,relres,iter]=IABSFILTERBANKREAL(...) additionally return the residuals in a
...
%   converge at all.  
%
%   IABSFILTERBANKREAL takes the following parameters at the end of the line of input
%   arguments:
%
...
% Check input paramameters.
    ...
    for iter=1:kv.maxit
      ...
      if flags.do_print
        if mod(iter,kv.printstep)==0
          fprintf('IABSFILTERBANKREAL: Iteration %i, residual = %f.\n',iter,relres(iter));
        end;    
      end;
  ...
  if flags.do_bfgs
    if exist('minFunc')~=2
      error(['To use the BFGS method in IABSFILTERBANKREAL, please install the minFunc ' ...
    ...
    f0 = 2*real(ifilterbank(c,gd,a));
    [f,fval,exitflag,output]=minFunc(@objfun,f0,opts,g,a,s);
...
function [f,df]=objfun(x,g,a,s)
  c = ufilterbank(x,g,a);
  
  inner=abs(c).^2-s;
  f=norm(inner,'fro')^2;
  
  df=8*real(ifilterbank(inner.*c,g,a));
